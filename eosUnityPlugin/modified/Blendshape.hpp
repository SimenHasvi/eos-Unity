...
/**
 * THIS WAS ADDED BY SIMEN
 * Loads blendshapes from a custom format which is easier to modify
 * @param filename
 * @return The blendshapes loaded
 */
inline std::vector<Blendshape> load_blendshapes_custom(std::string filename)
{
    std::vector<Blendshape> blendshapes;
    ...
    while (std::getline(file, line))
    {
        ...
        blendshape.deformation.resize(3448*3);
        blendshape.deformation.setZero();
        ...
        while (ss >> word)
        {
            ...
            {
                if (word == "#") break;
                blendshape.name = word;
            }
            else blendshape.deformation(i-1) = std::stof(word);
            i++;
        }
        if (word == "#") continue;
